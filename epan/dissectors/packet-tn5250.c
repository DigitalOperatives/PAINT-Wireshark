 ...
 *
 * $Id: packet-tn5250.c 35126 2010-12-06 01:34:58Z wmeier $
 *
...
static const value_string vals_tn5250_negative_responses[] = {
	...
	{ 0x10050149,  "Data follows an image/fax download command in the data stream and the image/fax download command does not contain the last  of the image data. No other commands are accepted until all the image/fax data has been received."},
...
static const struct true_false_string tn5250_field_wssf_ifc_flag1_0 = {
	"If the display supports a cache,this image/fax data remains in cache memory when the application sends one of the above commands to erase this image/fax data",
	"Erase this image/fax from memory whenever any of the following commands are received: CLEAR UNIT, CLEAR UNIT ALTERNATE (without saving image/fax), RESTORE RESTORE PARTIAL (if image/fax data was indicated in the SAVE PARTIAL), Another IMAGE/FAX CONTROL command"
};
...
static const value_string tn5250_vals_tn5250_wssf_ifc_vals[] = {
	...
	{ 0x02   ,  "Previously stored image/fax data is erased from the 5250 session and possibly from display memory (see bit 0 previously). All other parameters in this command are ignored."},
	{ 0x00, NULL }
};

static const struct true_false_string tn5250_field_wssf_ifc_flag1_4 = {
	"The display does not present scroll bars",
	"The display presents vertical and horizontal scroll bars as needed and allows the user to scroll without any interaction with the AS/400 application"
};
