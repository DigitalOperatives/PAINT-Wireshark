...
pipe_read_bytes(int pipe_fd, char *bytes, int required, char **msg)
{
    ...
    while(required) {
		...
		//DEBUG
		//printf("Read %d bytes from the dumpcap pipe\n", newly);
                ...
                char **err_msg)
{
    ...
    if(newly != 4) {
		...
		//DEBUG
		//printf("read %d failed to read header: %u\n", pipe_fd, newly);
	     ...
	     */
            *err_msg = g_strdup_printf("Premature EOF reading from sync pipe: got only %d bytes",
			...
			//DEBUG
			//printf("Premature EOF reading from sync pipe: got only %d bytes\n",newly);
        }
        return -1;
    }
    ...
    if(required == 0) {
		...
		//DEBUG
		//printf("read %d indicator: %c empty value\n", pipe_fd, *indicator);
    ...
    if(required > len) {
		...
		//DEBUG
		//printf("read %d length error, required %d > len %d, indicator: %u\n", pipe_fd, required, len, *indicator);
        ...
        *err_msg = g_strdup_printf("Unknown message from dumpcap, try to show it as a string: %s",
		...
		//DEBUG
		//printf("Err1 Unknown message from dumpcap, try to show it as a string: %s\n", msg);
    ...
    if(newly != required) {
    	if (newly != -1) {
            *err_msg = g_strdup_printf("Unknown message from dumpcap, try to show it as a string: %s",
			...
			//DEBUG
			//printf("Err2 Unknown message from dumpcap, try to show it as a string: %s\n", msg);
	}
        return -1;
    }
	...
	//DEBUG
	//printf("read %d ok indicator: %c len: %u msg: %s\n", pipe_fd, *indicator, len, msg);
